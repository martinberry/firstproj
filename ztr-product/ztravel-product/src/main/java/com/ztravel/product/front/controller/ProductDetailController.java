...
public class ProductDetailController {
	...
	public ModelAndView getProductDetail(@PathVariable("productPid") String productPid,
			...
			@RequestParam(required=false) String packageId, Model model,HttpServletRequest request) throws Exception {
		...
		String title = SEOUtil.getProductListSEO(product.getFrom(), product.getToCountry().get(0), SEOEnums.DETAIL_TITLE) ;
		title = StringUtils.isNotEmpty(title) ? title : "" ;
		model.addAttribute("title", product.getpName() + title);
		model.addAttribute("description", SEOUtil.getProductListSEO(product.getFrom(), product.getToCountry().get(0), SEOEnums.DETAIL_DESC));
		model.addAttribute("keywords", SEOUtil.getProductListSEO(product.getFrom(), product.getToCountry().get(0), SEOEnums.DETAIL_KEYWORDS));
		...
		return new ModelAndView("product/front/detail/detail_main") ;
	}
