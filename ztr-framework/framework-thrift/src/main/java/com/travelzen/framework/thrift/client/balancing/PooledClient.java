...
public abstract class PooledClient<T> implements Client<T> {
	...
	private ThriftInvocation thriftInvocation = new ThriftInvocation() {
		...
		public Object doBusiness(Invocation invocation) throws Throwable {
			logger.info("pool stats: numActive={}, numIdle={}.", pool.getNumActive(), pool.getNumIdle());
			Connection<T> conn = null;
			T client = null;
			try {
				conn = get();
				logger.info("get connection for business, conn:{}", conn);
				if (conn == null) {
					didFail();
			...
			try {
				...
				logger.info("do business, client:{}, conn:{}", client, conn);
...
class UnhealthyEvent extends ClientEvent {
	...
	public String toString() {
		return "UnhealthyEvent [time=" + time + "]";
	}
...
class HealthyEvent extends ClientEvent {
	...
	@Override
	public String toString() {
		return "HealthyEvent [time=" + time + ", unhealthyTime="
				+ unhealthyTime + "]";
	}
