...
public class ThriftClientPoolWithTreeCache<T> {
	...
	private void listenRpcServerClusterChange() throws Exception{
		final String rootZKPath = ZKPaths.makePath(basePath, rpcServiceName);
		treeCache = new TreeCache(TopsCuratorFramework.getInstance().getCuratorFramework(), rootZKPath);
		treeCache.getListenable().addListener(new TreeCacheListener(){

			@Override
			public void childEvent(CuratorFramework client, TreeCacheEvent event) throws Exception {
				logger.info(String.format("zookeeper节点[%s]发生变动， event:%s", rootZKPath, event));
	...
	private void initClientPool() {
		...
		KeyedPoolableObjectFactory<BackendRpcAddress, T> objectFactory = new KeyedPoolableObjectFactory<BackendRpcAddress, T>() {
			...
			public T makeObject(BackendRpcAddress key) throws Exception {
				...
				Class serviceClient = ThriftClientPoolWithTreeCache.this.getServiceClientClass();
	...
	private class FailRetryInvocationHandler implements InvocationHandler {
		...
		private RetryTemplate createRetryTemplate() {
			...
			maxAttemptRetryPolicy.setMaxAttempts(10);
		...
		public Object invoke(Object proxy, final Method method, final Object[] args) throws Throwable {
			RetryCallback<Object> worker = new RetryCallback<Object>() {
				public Object doWithRetry(RetryContext retryContext) throws Exception {
					...
					try {
						address = ThriftClientPoolWithTreeCache.this.backendRpcAddressPool.next();
						if(address == null)
							throw new Exception("没有可用的后端服务, backendRpcAddressPool is empty");
						handlerLogger.info("当前使用的rpc地址:" + address);
						serviceClient = ThriftClientPoolWithTreeCache.this.clientPool.borrowObject(address);
						...
						if(ExceptionUtils.indexOfThrowable(e, TTransportException.class) != -1)
							ThriftClientPoolWithTreeCache.this.clientPool.invalidateObject(address, serviceClient);
						else retryContext.setExhaustedOnly();
						throw e;
					} finally{
						if(address != null && serviceClient != null)
							ThriftClientPoolWithTreeCache.this.clientPool.returnObject(address, serviceClient);
					}
				}
			};
	...
	private class BackendRpcAddressPool{
		...
		private synchronized void refresh(){
			...
			}else poolLogger.info("后端服务节点没有变化");
		}
		...
		private List<BackendRpcAddress> getBackendRpcAddressListFromZK(){
			...
			}else addresses = TopsZookeeperBalancer.getRpcAddress(rpcServiceName, basePath, false);
