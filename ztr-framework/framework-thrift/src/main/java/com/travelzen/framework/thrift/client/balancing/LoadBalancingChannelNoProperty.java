...
public abstract class LoadBalancingChannelNoProperty<T> implements Client<T> {
	...
	public LoadBalancingChannelNoProperty(List<String> list, String prefix, String serviceName) {
		this(list, prefix, serviceName, false);
	}

	/**
	 * list代表的是服务端地址，每个地址必须符合这样的格式: ip:port,譬如：192.168.163.54:9860
	 * serviceName必须是跟服务端的服务名一致。null表示取消无缝切换功能
	 *   needCache  是否缓存zk上的server列表
	 * @param list
	 * @param servcieName
	 */
	public LoadBalancingChannelNoProperty(List<String> list, String prefix, String serviceName, boolean needCache) {
		...
		if (serviceName != null && serviceName.length() > 0 && prefix != null && prefix.length() > 0) {
			...
			this.prefix = prefix;
			TopsZookeeperBalancer.getRpcAddress(serviceName, prefix, needCache);
		}
	...
	public T proxy() throws Exception {
		...
		if (isUseCurator) {// 如果使用curator
			List<String> rpcList = TopsZookeeperBalancer.getRpcAddress(serviceName, prefix, false);
	...
	public abstract Client<T> createThriftCient(String ip, int port);

	protected synchronized Client<T> createThriftCient(String serverName) {
		if (serverName == null)
			return null;
		// 做一次检查
		...
		if (params.length == 2) {
			...
			if (port > 0) {
				client = createThriftCient(ip, port);
				if (client != null) {
	...
	private Client<T> getClient(String serverName) {
		...
		Client<T> client = underlyingMap.get(serverName);
		if (client == null) {
			client = createThriftCient(serverName);
		}
		return client;
	}
}
