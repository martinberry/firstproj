	    ...
	    $(this).parents("form").ajaxSubmit({
			...
			success : function(response) {
				if(response.res_code == 'SO_PROD_1001'){
					...
					if(pictureId){
						$($this).parents("form[name=pictureform]").find("[name-value='pictureId']").val(pictureId);
						if($($this).hasClass("againUpload")){
							$($this).parents(".upLoadProductImg").find("img").attr("src",mediaserver+"imageservice?mediaImageId="+pictureId);
						}else if($($this).hasClass("instantUploadFile")){
							var imageHtml ='<div class="upLoadProductImg upload"> '
							...
							+'<input type ="hidden" name-value="pictureId" value="'+pictureId+'" class="giftImage" name="giftImage"/>'
							+' <a href="javascript:void(0);" class="againUploadFonts">重新上传</a>'
							+"</div>"
							+'</form>'
							+'</div>';	
							$($this).parents(".instantUpload").replaceWith(imageHtml);
						}
