...
public class WxOrderDetailController {
	...
	public String detail(@PathVariable String orderId,Model model){
		...
		if(null != memberSessionBean){
			...
			try {
				...
				ProductHotelInfoListVo hotelInfoListVo = wxOrderDetailServiceImpl.getProductHotelInfoListVoByOrder(orderProduct.getProductSnapshot());
				model.addAttribute("hotelInfoListVo", hotelInfoListVo);
				FlightInfoVo flightInfoVo = wxOrderDetailServiceImpl.getFlightInfoVoByOrder(orderProduct.getProductSnapshot(),orderProduct.getBookDate());
				...
				List<AdditionalProductWo> additionalProducts = wxOrderDetailServiceImpl.getAdditionalProductsBySnapshot(orderProduct.getProductSnapshot());
				model.addAttribute("additionalProducts", additionalProducts);
			} catch (Exception e) {
				LOGGER.error("查询订单[{}]详情异常:{}", orderId,e);
			}
		}
		return "order/weixin/orderDetail/detail";
	}
	...
	@RequestMapping(value="/detail/getBedPrefer")
	public String getBedPrefer(@RequestBody String orderId,Model model){
		MemberSessionBean memberSessionBean = SSOUtil.getMemberSessionBean();
		if(null != memberSessionBean){
			String mid = memberSessionBean.getMid();
			Map<String,String> params = OrderConvert.getDetailParams(mid,orderId);
			try {
				...
				Boolean canEdit = wxOrderServiceImpl.isOrderCanEdit(orderProduct.getBookDate(),order.getBackState());
				model.addAttribute("canEdit", canEdit);
				model.addAttribute("isBedTips", wxOrderServiceImpl.isBedTip(orderProduct.getProductId()));
				model.addAttribute("bedPrefer", order.getBedPrefer());
			} catch (Exception e) {
				LOGGER.error("查询订单[{}]床型偏好信息异常:{}", orderId,e);
			}
		}
		return "order/weixin/orderDetail/bedPrefer";
	}


	@RequestMapping(value="/detail/updateBedPrefer", method = RequestMethod.POST)
	@ResponseBody
	 public AjaxResponse updateBedPrefer(@RequestParam String bedPrefer, String orderId) {
		try{
			...
			Map<String,String> params = OrderConvert.getDetailParams(SSOUtil.getMemberSessionBean().getMid(),orderId);
			Order order = orderReuseService.selectByMap(params);
			if(order == null || order.getBackState().equals(ZtOrderStatus.CANCELED.name())|| order.getBackState().equals(ZtOrderStatus.REFUNDFAILED.name()) || order.getBackState().equals(ZtOrderStatus.REFUNDING.name())){
				return AjaxResponse.instance(Const.FO_ORDE_CODE_1020, Const.FF_ORDE_REASON_1020);
			}
