...
$(function(){
	...
	$(".bigBlue11b9b7Btn").click(function(){
		...
		var password2 = trim($("#againPasswordInputer")) ;
		...
		if(!submit_lock){
			...
			$.ajax({
				type: 'POST',
				url: ssoServer + '/sso/resetPasswordByEmail' ,
				...
				success: function(data){
					...
					if(data.res_code == 'EF_MEMB_1032'){
						...
						toggleErrorHint(disappear) ;
						toggleErrorHint(password_hint_error2) ;
					}else if(data.res_code == 'EF_MEMB_1034'){
						toggleErrorHint(disappear) ;
						toggleErrorHint(password_hint_error3) ;
					}else if(data.res_code == 'SF_MEMB_1008'){
						toggleErrorHint(disappear) ;
						jumpTo = data.res_msg ;
