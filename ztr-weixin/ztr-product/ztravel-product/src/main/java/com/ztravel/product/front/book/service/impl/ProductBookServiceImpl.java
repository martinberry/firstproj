package com.ztravel.product.front.book.service.impl;
...
@Service
public class ProductBookServiceImpl implements IProductBookService{

	private static Logger LOGGER = RequestIdentityLogger.getLogger(ProductBookServiceImpl.class);
	...
	public AjaxResponse applyOrder(ProductBookItem productBookItem)throws Exception {
		...
		if(applyOrderResult.getRes_code().equals(ProductBookConstans.USER_NOT_LOGIN) || applyOrderResult.getRes_code().equals(ResponseConstants.MEMB_MOBILE_NOT_FOUND_ERROR_CODE)){
			return applyOrderResult;//用户未登录或者未注册
		}
		//验证产品库存
		...
		if(checkPriceByProduct(productBookItem) == 1) {
			Double price = getTotalPriceByProductBookItem(productBookItem);
	...
	public Integer checkPriceByProduct(ProductBookItem productBookItem) throws Exception {
		Double totalPrice = getTotalPriceByProductBookItem(productBookItem);
	...
	public static void main(String args[]){
		Properties mailProp = TopsConfReader.getConfProperties("properties/mail.properties", ConfScope.R) ;
		String e = mailProp.getProperty("mail.service","");
	...
	public void opsAfterApplied(AjaxResponse applyOrderResult,ProductBookItem productBookItem,String orderNo) throws Exception{
		...
		systemNoticeClientServiceImpl.add(member.getMemberId(), NoticeType.ORDERTYPE, SystemNoticeContentUtil.getOdContent(1, member.getNickName(), applyOrderResult.getRes_msg(),productBookItem.getNature()));
	}
...
//			ssoClientService.doRegisterAndSendPassword(entity, true) ;
			return 1;
		}
	}
...
//						travelInfoClientSeriveImpl.save(traveller);
	...
	public MailEntity buildMailEntity(ProductBookItem productBookItem,String orderNo,String orderId){
		...
		String url = WebEnv.get("server.path.media", "")+"/imageservice?mediaImageId=" + productBookItem.getFirstImageId();//need to change
...
//		String email = productBookItem.getContactorInfo().getEmail();
	...
	@Override
	public ContactorInfo getContactorInfo() throws Exception {
		if(getLoginIngo()){
			return orderClientService.getContactorInfoByMemId(SSOUtil.getMemberSessionBean().getMemberId());
		}else{
			return null;
		}
	}
	
	public List<OrderComContactor> getComContactors() throws Exception {
		List<OrderComContactor> comContactors = null;
		if(getLoginIngo()){
			String memberId = SSOUtil.getMemberSessionBean().getMemberId();
			comContactors = orderClientService.getContactorsByMemId(memberId);
		}
		return comContactors;
	}
	 ...
	 * @param dayInfo
	 */
	private void setAdultAndChildPriceInfo(ProductBookVo bookVo, DetailToOrderCriteria criteria, Day dayInfo) {
        ...
        bookVo.setTotalPrice(moneyFormat.format(totalAdultPrice + totalChildPrice + totalSinglePrice));
	}
	 ...
	 * @param dayInfo
	 */
	private void setPackagePriceInfo(ProductBookVo bookVo, DetailToOrderCriteria criteria, Day dayInfo) {

        Double totalPrice = 0d;
        if(null != dayInfo.getSale()){
            boolean existsPackageFlag = false;
            //遍历当日套餐，按套餐id获取套餐名称
            for (SalesPackage salesPackage : dayInfo.getSale().getSalesPackages()) {
            ...
            Assert.isTrue(existsPackageFlag, "产品{"+criteria.getProductId()+"}在日期为{"+criteria.getBookDate()+"}时查询不到{"+criteria.getPackageId()+"}套餐信息");
        }
	...
	public ProductBookVo freeTravelProduct2BookVo(DetailToOrderCriteria criteria)throws Exception{
		...
		if (StringUtil.isEmpty(criteria.getPackageId())) {
		    setAdultAndChildPriceInfo(bookVo, criteria, dayInfo);
		} else {
		    setPackagePriceInfo(bookVo, criteria, dayInfo);
		}
			...
			if(!CollectionUtils.isEmpty(flightInfoList)){
				for(FlightInfo flightInfo : flightInfoList){
					...
					if(flightInfo.getAirRange().getName().equals(AirRange.GO.getName())){
						productFlight.setDepartDate(DateUtils.addDate(criteria.getBookDate(), 3, flightInfo.getOffsetDays() == null ? 0 : flightInfo.getOffsetDays() - 1));
						...
						productFlight.setDepartDate(DateUtils.addDate(criteria.getBookDate(), 3, flightInfo.getOffsetDays() == null ? 0 : flightInfo.getOffsetDays() - 1));//中间程的航班出发日期待定
    ...
    @Override
    public Double getProductPackagePrice(String productId, String bookDate,
    ...
    @Override
    public List<CouponClientEntity> getAllCoupons(String productId, String bookDate, String packageId) {
        ...
        try {
            ...
            if(null != memberId){
                String productNo = productClientService.getProductById(productId).getProductCode();
                Double totalPrice = getProductPackagePrice(productId, bookDate, packageId);
                ...
                if(!CollectionUtils.isEmpty(items)){
                    for(CouponItemBean itemBean : items){
                        CouponClientEntity couponClientEntity = activityClientService.getCouponByIdWithProductName(itemBean.getActivityId(), itemBean.getCouponCode());
                        if(!StringUtils.isEmpty(couponClientEntity.getCouponId())){
                            ...
                            if(couponClientEntity.getProductRangeType().equals(ProductRangeType.ALLPRODUCTS.name())){
                                ...
                                if(totalPrice.longValue() >= couponClientEntity.getOrderLeast() && Arrays.asList(couponClientEntity.getProductRange().split(",")).indexOf(productNo) >= 0){
                                    couponClientEntityList.add(couponClientEntity);
                                }
                            }
                        }
    ...
    public Double getAdultChildTotalPrice(String productId, String bookDate, Integer adultNum, Integer childNum) throws Exception {
        Double[] priceArray = getProductPriceInfo(productId, bookDate);
        Double totalPrice = 0D ;
    ...
    public Double[] getProductPriceInfo(String productId, String bookDate)throws Exception {
        ...
        Sale sale = product.getCalendar().get(bookDate).getSale() ;
        if(null != sale){
            ret[0] = sale.getAdultPrice() ;
            ret[1] = sale.getChildPrice() == null ? sale.getAdultPrice() : sale.getChildPrice() ;
            ret[2] = sale.getSingleRoomPrice() ;
        }
        return ret ;
    }

    @Override
    public List<CouponClientEntity> getAllCoupons(String productId,String bookDate,Integer adultNum,Integer childNum,String productNature,String priceId){
        String memberId = null;
        List<CouponClientEntity> couponClientEntityList = new ArrayList<>();
        List<CouponItemBean> items = null;
        try {
            ...
            if(null != memberId){
            	...
            	if(productNature.equals(Nature.COMBINATION.name()) || productNature.equals(Nature.PACKAGE.name())){
            		...
            		totalPrice = getAdultChildTotalPrice(productId, bookDate, adultNum, childNum);
            	}else{
    ...
    public Double getTotalPriceByProductBookItem(ProductBookItem productBookItem) throws Exception {
    	if(Nature.PACKAGE.name().equals(productBookItem.getNature()) || Nature.COMBINATION.name().equals(productBookItem.getNature())){
    		if (StringUtil.isEmpty(productBookItem.getPackageId())) {
    			return getAdultChildTotalPrice(productBookItem.getProductId(), productBookItem.getBookDate(), productBookItem.getAdaultNum(), productBookItem.getChildrenNum());
    		} else {
    			return getProductPackagePrice(productBookItem.getProductId(), productBookItem.getBookDate(), productBookItem.getPackageId());
    		}
    	}else{
