...
public class SpringDAOTemplate extends AbstractDAOTemplate {
    ...
    protected void configureDeleteMethodTemplate() {
        setDeleteMethodTemplate("this.getSqlMapClientTemplate().delete(\"{0}.{1}\", {2});"); //$NON-NLS-1$
    }

    @Override
    protected void configureInsertMethodTemplate() {
        setInsertMethodTemplate("this.getSqlMapClientTemplate().insert(\"{0}.{1}\", {2});"); //$NON-NLS-1$
    }

    @Override
    protected void configureQueryForListMethodTemplate() {
        setQueryForListMethodTemplate("this.getSqlMapClientTemplate().queryForList(\"{0}.{1}\", {2});"); //$NON-NLS-1$
    }

    @Override
    protected void configureQueryForObjectMethodTemplate() {
        setQueryForObjectMethodTemplate("this.getSqlMapClientTemplate().queryForObject(\"{0}.{1}\", {2});"); //$NON-NLS-1$
    }
    ...
    protected void configureUpdateMethodTemplate() {
        setUpdateMethodTemplate("this.getSqlMapClientTemplate().update(\"{0}.{1}\", {2});"); //$NON-NLS-1$
    }
}
