...
import org.apache.ibatis.ibator.generator.ibatis2.sqlmap.elements.PostgresPaginationLimitGenerator;
...
public class PaginationPlugin extends IbatorPluginAdapter {
	...
	public boolean validate(List<String> warnings) {
		databaseType = ibatorContext.getJdbcConnectionConfiguration().getDriverClass();
		return true;
	}
	...
	private void element(XmlElement element) {
		if (databaseType.contains("oracle")) {
			...
			oracleHeadIncludeElement.addAttribute(new Attribute("refid", XmlConstants.COMMON + "." + OraclePaginationHeadGenerator.ORACLE_PAGINATION_HEAD));
			// 在第一个地方增加
			element.addElement(0, oracleHeadIncludeElement);

			XmlElement oracleTailIncludeElement = new XmlElement("include");
			oracleTailIncludeElement.addAttribute(new Attribute("refid", XmlConstants.COMMON + "." + OraclePaginationTailGenerator.ORACLE_PAGINATION_TAIL));
			// 在最后增加
			...
			mysqlLimitIncludeElement.addAttribute(new Attribute("refid", XmlConstants.COMMON + "." + MysqlPaginationLimitGenerator.MYSQL_PAGINATION_LIMIT));
			// 在最后增加
			element.addElement(element.getElements().size(), mysqlLimitIncludeElement);
		} else if (databaseType.contains("postgres")) {
			XmlElement postgresLimitIncludeElement = new XmlElement("include");
			postgresLimitIncludeElement.addAttribute(new Attribute("refid", XmlConstants.COMMON + "." + PostgresPaginationLimitGenerator.POSTGRES_PAGINATION_LIMIT));
			// 在最后增加
			element.addElement(element.getElements().size(), postgresLimitIncludeElement);
		}
	}
	...
	public boolean modelExampleClassGenerated(TopLevelClass topLevelClass, IntrospectedTable introspectedTable) {
		if (databaseType.contains("oracle")) {
			...
			// add field, getter, setter for pgOffset clause
			Field field = new Field();
			field.setVisibility(JavaVisibility.PRIVATE);
			field.setType(FullyQualifiedJavaType.getInteger());
			field.setName("pgOffset"); //$NON-NLS-1$
			topLevelClass.addField(field);

			Method method = new Method();
			method.setVisibility(JavaVisibility.PUBLIC);
			method.setName("setPgOffset"); //$NON-NLS-1$
			method.addParameter(new Parameter(FullyQualifiedJavaType.getInteger(), "pgOffset")); //$NON-NLS-1$
			method.addBodyLine("this.pgOffset = pgOffset;"); //$NON-NLS-1$
			addSetterComment(method, "指定返回记录行的偏移量<br> pgOffset= 5,pgLength=10;  // 检索记录行 6-15", "pgOffset");
			topLevelClass.addMethod(method);

			method = new Method();
			method.setVisibility(JavaVisibility.PUBLIC);
			method.setReturnType(FullyQualifiedJavaType.getInteger());
			method.setName("getPgOffset"); //$NON-NLS-1$
			method.addBodyLine("return pgOffset;"); //$NON-NLS-1$
			topLevelClass.addMethod(method);
			// add field, getter, setter for pgLength clause
			field = new Field();
			field.setVisibility(JavaVisibility.PRIVATE);
			field.setType(FullyQualifiedJavaType.getInteger());
			field.setName("pgLength"); //$NON-NLS-1$
			topLevelClass.addField(field);

			method = new Method();
			method.setVisibility(JavaVisibility.PUBLIC);
			method.setName("setPgLength"); //$NON-NLS-1$
			method.addParameter(new Parameter(FullyQualifiedJavaType.getInteger(), "pgLength")); //$NON-NLS-1$
			method.addBodyLine("this.pgLength = pgLength;"); //$NON-NLS-1$
			addSetterComment(method, "指定返回记录行的最大数目<br> pgOffset= 5,pgLength=10;  // 检索记录行 6-15", "pgLength");
			topLevelClass.addMethod(method);

			method = new Method();
			method.setVisibility(JavaVisibility.PUBLIC);
			method.setReturnType(FullyQualifiedJavaType.getInteger());
			method.setName("getPgLength"); //$NON-NLS-1$
			method.addBodyLine("return pgLength;"); //$NON-NLS-1$
			topLevelClass.addMethod(method);
			// 增加结束处
		}
	...
	public List<GeneratedXmlFile> contextGenerateAdditionalXmlFiles() {
		...
		if (databaseType.contains("oracle")) {
			...
			// 增加postgres
			AbstractXmlElementGenerator postgresPaginationLimitGenerator = new PostgresPaginationLimitGenerator();
			postgresPaginationLimitGenerator.setIbatorContext(ibatorContext);
			postgresPaginationLimitGenerator.addElements(answer);
		}
		...
		GeneratedXmlFile gxf = new GeneratedXmlFile(document, properties.getProperty("fileName", XmlConstants.COMMON + "_SqlMap.xml"), //$NON-NLS-1$ //$NON-NLS-2$
				ibatorContext.getSqlMapGeneratorConfiguration().getTargetPackage(), //$NON-NLS-1$
				ibatorContext.getSqlMapGeneratorConfiguration().getTargetProject(), //$NON-NLS-1$
				false);
