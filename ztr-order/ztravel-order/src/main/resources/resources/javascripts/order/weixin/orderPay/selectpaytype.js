...
$(function(){
	...
	//点击支付按钮
	$(".goPay").delegate(".pay-link","click",function(){
		...
		if(!lock){
			...
			if(paymentType == 'WeChatpay'){
				$.ajax({
					type : "POST",
					url : wxServer+"/weixin/orderPay/jumpToWechatPay",
					data :{
					...
					success : function(result) {
						...
						if(paymentResponse.success){
							...
							window.location.href = wxServer + "/weixin/orderPay/payfailed?orderId=" + orderId ;
						}
				...
				$.ajax({
					type : "POST",
					url : wxServer+"/weixin/orderPay/jumpToAliPay",
					data :{
						paymentType:paymentType,
						payAmount:payAmount,
						orderId:orderId,
					...
					success : function(result) {
						...
						if(paymentResponse.success && (paymentResponse.paymentUrl != null && paymentResponse.paymentUrl!='')){
							...
							if(is_weixin()){
								...
								window.location.href = wxServer + "/weixin/orderPay/blank?url=" + payUrl ;
							}else{
    ...
    WeixinJSBridge.invoke(
       ...
       function(res){
           if(res.err_msg == "get_brand_wcpay_request:ok" ) {
        	   ...
        	   window.location.href = wxServer + "/weixin/orderPay/payfailed?orderId=" + $("#orderId").val() ;
           }
       }
   );
}
